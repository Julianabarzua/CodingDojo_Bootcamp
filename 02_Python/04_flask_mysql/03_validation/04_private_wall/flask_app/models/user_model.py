...
class User:
    ...
    def logedUser(cls, data):
        ...
        """
        result = connectToMySQL('wall_schema').query_db(query, data)
        return result
    ...
    def save_message(cls, data ):
        query = "INSERT INTO messages (id_from, id_to, message, created_at, updated_at ) VALUES (%(from_id)s,%(to_id)s,%(content)s, NOW() , NOW() );"
        return connectToMySQL('wall_schema').query_db( query, data )
